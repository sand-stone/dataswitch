...
package slipstream.replicator.applier.batch;
...
import slipstream.replicator.commons.common.csv.CsvException;
import slipstream.replicator.commons.common.csv.CsvWriter;
import slipstream.replicator.ReplicatorException;
import slipstream.replicator.applier.RawApplier;
import slipstream.replicator.consistency.ConsistencyException;
import slipstream.replicator.consistency.ConsistencyTable;
import slipstream.replicator.csv.CsvDataFormat;
import slipstream.replicator.csv.CsvFile;
import slipstream.replicator.csv.CsvFileSet;
import slipstream.replicator.csv.CsvInfo;
import slipstream.replicator.csv.CsvKey;
import slipstream.replicator.database.Column;
import slipstream.replicator.database.Database;
import slipstream.replicator.database.Key;
import slipstream.replicator.database.Table;
import slipstream.replicator.database.TableMetadataCache;
...
import slipstream.replicator.datatypes.Numeric;
...
import slipstream.replicator.dbms.RowChangeData.ActionType;
...
import slipstream.replicator.scripting.ScriptMethodRequest;
import slipstream.replicator.scripting.ScriptMethodResponse;
import slipstream.replicator.scripting.SqlWrapper;
...
public class SimpleBatchApplier implements RawApplier
{
            ...
            else if (dbmsData instanceof RowChangeData)
            {
                ...
                for (OneRowChange orc : rd.getRowChanges())
                {
                    ...
                    if (action.equals(ActionType.INSERT))
                    {
                        ...
                        List<OneRowChange.ColumnSpec> colSpecs = orc.getColumnSpec();
                        ArrayList<ArrayList<OneRowChange.ColumnVal>> colValues = orc
                                .getColumnValues();
                        // PK should be put in here by the PrimaryKeyFilter.
                        List<OneRowChange.ColumnSpec> keySpecs = orc.getKeySpec();
     ...
     */
    private boolean keysUnchanged(List<OneRowChange.ColumnSpec> keySpecs,
                                  ArrayList<ArrayList<OneRowChange.ColumnVal>> keyValues,
                                  List<OneRowChange.ColumnSpec> colSpecs, ArrayList<ArrayList<OneRowChange.ColumnVal>> colValues)
    {
        // Iterate over the key values.
        for (int k = 0; k < keySpecs.size(); k++)
        {
            // Find the key and matching column specifications.
            OneRowChange.ColumnSpec keySpec = keySpecs.get(k);
            int keyIndex = keySpec.getIndex() - 1;
            OneRowChange.ColumnSpec colSpec = null;
            ...
            {
                OneRowChange.ColumnVal keyValueHolder = keyValues.get(row).get(k);
                Object keyValue = keyValueHolder.getValue();

                // Is corresponding column value different from key value?
                OneRowChange.ColumnVal colValueHolder = colValues.get(row).get(keyIndex);
            ...
            InterruptedException
    {
        ...
        for (int i = 0; i < stageColumnNames.size(); i++)
        {
            ...
            OneRowChange.ColumnSpec headerColSpec = stageHeader.new ColumnSpec();
    ...
    private void writeValues(long seqno, Timestamp commitTimestamp,
                             String service, Table tableMetadata, List<OneRowChange.ColumnSpec> colSpecs,
                             ArrayList<ArrayList<OneRowChange.ColumnVal>> colValues, String opcode)
            throws ReplicatorException
    {
        ...
        try
        {
            ...
            Iterator<ArrayList<OneRowChange.ColumnVal>> colIterator = colValues.iterator();
            while (colIterator.hasNext())
            {
                ...
                for (int i = 0; i < headerSize; i++)
                {
                    ...
                    OneRowChange.ColumnSpec headerColSpec = stageHeader.getColumnSpec().get(
                            i);
                ...
                ArrayList<OneRowChange.ColumnVal> row = colIterator.next();
                for (int i = 0; i < row.size(); i++)
                {
                    // Get the column and its type value.
                    OneRowChange.ColumnVal columnVal = row.get(i);
                    OneRowChange.ColumnSpec columnSpec = colSpecs.get(i);
    ...
    private Table getTableMetadata(String schema, String name,
                                   List<OneRowChange.ColumnSpec> colSpecs, List<OneRowChange.ColumnSpec> keySpecs)
    {
        ...
        if (t == null)
        {
            ...
            for (OneRowChange.ColumnSpec colSpec : colSpecs)
            {
     ...
     */
    protected String getCsvString(Object value, OneRowChange.ColumnSpec columnSpec)
            throws ReplicatorException
    {
