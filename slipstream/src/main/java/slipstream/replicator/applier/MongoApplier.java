...
public class MongoApplier implements RawApplier
{
            ...
            else if (dbmsData instanceof RowChangeData)
            {
                ...
                for (OneRowChange orc : rd.getRowChanges())
                {
                    ...
                    if (action.equals(ActionType.INSERT))
                    {
                        ...
                        Iterator<ArrayList<OneRowChange.ColumnVal>> colValues = orc
                                .getColumnValues().iterator();
                        while (colValues.hasNext())
                        {
                            BasicDBObject doc = new BasicDBObject();
                            ArrayList<OneRowChange.ColumnVal> row = colValues.next();
                    ...
                    else if (action.equals(ActionType.UPDATE))
                    {
                        ...
                        {
                            List<OneRowChange.ColumnVal> keyValuesOfRow = keyValues.get(row);
                            List<OneRowChange.ColumnVal> colValuesOfRow = columnValues
                                    .get(row);
     ...
     */
    private void setValue(DBObject doc, OneRowChange.ColumnSpec columnSpec, Object value)
            throws ReplicatorException
    {
