...
public class PrimaryKeyFilter implements Filter
{
    ...
    private void checkForPK(OneRowChange orc) throws SQLException
    {
        ...
        // from the row event
        ArrayList<OneRowChange.ColumnSpec> keySpecs = orc.getKeySpec();
        ArrayList<ArrayList<OneRowChange.ColumnVal>> keyValues = orc.getKeyValues();

        for (Iterator<OneRowChange.ColumnSpec> iterator = keySpecs.iterator(); iterator
                .hasNext();)
        {
            OneRowChange.ColumnSpec keySpec = iterator.next();
            boolean found = false;
            ...
            if (!found)
            {
                ...
                for (Iterator<ArrayList<OneRowChange.ColumnVal>> iterator2 = keyValues
                        .iterator(); iterator2.hasNext();)
                {
                    ...
                    ArrayList<OneRowChange.ColumnVal> values = iterator2.next();
                    values.remove(idx);
                }
        ...
        if (orc.getAction() == ActionType.INSERT)
        {
            ...
            if (keySpecs.size() == 0 && keyValues.size() == 0)
            {
                ...
                ArrayList<OneRowChange.ColumnVal> columnValues = new ArrayList<OneRowChange.ColumnVal>();
