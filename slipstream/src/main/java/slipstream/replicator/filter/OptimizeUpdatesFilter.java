...
public class OptimizeUpdatesFilter implements Filter
{
    ...
    private boolean filterStaticColumns(OneRowChange orc) throws ReplicatorException
    {
        ...
        for (int k = 0; k < keys.size(); k++)
        {
            ...
            {
                OneRowChange.ColumnVal keyValue = keyValues.get(row).get(k);

                // Is corresponding column value different from key's
                // (i.e. current)?
                OneRowChange.ColumnVal colValue = columnValues.get(row).get(valIndex);
        ...
        for (Iterator<OneRowChange.ColumnSpec> iterator = columnsToRemove.iterator(); iterator
                .hasNext();)
        {
            OneRowChange.ColumnSpec columnToRemoveSpec = iterator.next();
            ...
            for (Iterator<ArrayList<OneRowChange.ColumnVal>> iterator2 = columnValues
