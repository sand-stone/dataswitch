...
import slipstream.replicator.commons.common.cache.IndexedLRUCache;
...
import slipstream.replicator.database.TableMatcher;
        ...
        for (DBMSData dataElem : data)
        {
            if (dataElem instanceof RowChangeData)
            {
                ...
                for (OneRowChange orc : rdata.getRowChanges())
                {
                    ...
                    ArrayList<OneRowChange.ColumnSpec> keysToRemove = new ArrayList<OneRowChange.ColumnSpec>();
                    ArrayList<OneRowChange.ColumnSpec> columnsToRemove = new ArrayList<OneRowChange.ColumnSpec>();
                    ...
                    {
                        OneRowChange.ColumnSpec keySpec = keys.get(k);
                    ...
                    for (Iterator<OneRowChange.ColumnSpec> iteratorRemoveKey = keysToRemove.iterator(); iteratorRemoveKey
                            .hasNext();)
                    {
                        OneRowChange.ColumnSpec keyToRemoveSpec = iteratorRemoveKey.next();
                        int idx = keys.indexOf(keyToRemoveSpec);
                        
                        for (Iterator<ArrayList<OneRowChange.ColumnVal>> iteratorKeyValue = keyValues
                                .iterator(); iteratorKeyValue.hasNext();)
                        {
                            ArrayList<OneRowChange.ColumnVal> kValues = iteratorKeyValue.next();
                            kValues.remove(idx);
                        }
                    ...
                    // Remove unwanted columns now that we know which those are.
                    for (Iterator<OneRowChange.ColumnSpec> iteratorRemoveColumn = columnsToRemove.iterator(); iteratorRemoveColumn
                            .hasNext();)
                    {
                        OneRowChange.ColumnSpec columnToRemoveSpec = iteratorRemoveColumn.next();
                        int idx = columns.indexOf(columnToRemoveSpec);

                        // Iterate through each row.
                        for (Iterator<ArrayList<OneRowChange.ColumnVal>> iteratorValue = columnValues
                                .iterator(); iteratorValue.hasNext();)
                        {
                            ArrayList<OneRowChange.ColumnVal> cValues = iteratorValue.next();
                            cValues.remove(idx);
                        }
