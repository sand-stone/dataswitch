...
import slipstream.replicator.database.DatabaseHelper;
...
import slipstream.replicator.extractor.mysql.conversion.BigEndianConversion;
import slipstream.replicator.extractor.mysql.conversion.GeneralConversion;
import slipstream.replicator.extractor.mysql.conversion.LittleEndianConversion;
...
public abstract class RowsLogEvent extends LogEvent
{
    ...
    protected int extractValue(OneRowChange.ColumnSpec spec, OneRowChange.ColumnVal value, byte[] row,
        ...
        for (int i = 0; i < map.getColumnsCount(); i++)
        {
            ...
            if (isKeySpec)
            {
                ...
                else
                {
                    ...
                    OneRowChange.ColumnSpec keySpec = oneRowChange.getKeySpec()
                            .get(colCount);
                    if (keySpec != null
            ...
            else
            {
                ...
                {
                    // Check if column was null until now
                    OneRowChange.ColumnSpec columnSpec = oneRowChange.getColumnSpec().get(
                            colCount);
