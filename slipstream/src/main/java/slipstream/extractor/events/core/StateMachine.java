...
public class StateMachine
{
    ...
    private volatile State            pendingState;
            ...
            throws FiniteStateException, InterruptedException
    {
        ...
        try
        {
            // Set pending state variable to signal a state transition to
            // clients.
            ...
            if (state != nextState)
            {
                ...
                if (isForwardChainEnabled())
                {
                    ...
                    try
                    {
                        if ((transition = map.nextTransition(state, event,
                                entity)) != null)
                        {
                            applyEvent(event);
                        }
    ...
    /**
     * Returns the next state if a transition is currently being processed or
     * null if no state transition is currently in effect. This call is
     * non-blocking to set the state.
     */
